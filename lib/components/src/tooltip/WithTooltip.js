...
const WithTooltip = lifecycle({
  componentDidMount() {
    ...
    iframes.forEach(iframe => {
      const bind = () => {
        if (iframe.contentDocument) iframe.contentDocument.addEventListener('click', hide);
        unbinders.push(() => {
          if (iframe.contentDocument) iframe.contentDocument.removeEventListener('click', hide);
        });
      };
